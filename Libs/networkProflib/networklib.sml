...
structure Network : NETWORK = struct
    ...
    fun listen () = 
        let fun listen fb al = 
                let fun pktIDCmp (PktID pktID1) (PktID pktID2) = 
                        let fun ipCmp [] [] = true
                    ...
                    (* val fragmentBuffer : (packetID * (fragment list ref)) list ref = ref fb

                    val assemblingList : (packetID * (char array)) list ref = ref al *)
            ...
            in
                if !times < !runs then (
                    let val ethFrame = receive () 
                        val (ethHeader, ethPayload) = Eth.decode ethFrame
