...
pp_gen(Gen *gen)
{
  ...
  for (rp = clear_fp(gen->fp) ; rp ; rp = clear_tospace_bit(rp->n)) {
    printk("  Rp %p, next:%p, data: %p, rp+1: %p\n",
	    rp,
	    rp->n,
	    &(rp->i),
	    rp+1);
...
{
  r = clearStatusBits(r);
  printk("printRegionInfo called from: %s\n",str);
  printk("Region at address: %p\n", r);
  pp_gen(&(r->g0));
  return;
}
...
chk_obj_in_gen(Gen *gen, uintptr_t *obj_ptr, char* s)
{
  ...
  if (! found) {
    printk("chk_obj_in_gen, obj_ptr: %p not in gen:\n",obj_ptr);
...
void callSbrk() {
  ...
  if ( sb == NULL ) {
    printk("I could not allocate more memory; either no more memory is\navailable or the memory subsystem is detectively corrupted\n");
    return;
  }
...
// Region __resetRegion (Region rAdr) {
//   return resetRegion(rAdr);
// }
